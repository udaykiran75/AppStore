...
class AppStore extends Component {
  ...
  render() {
    const {tabItemId} = this.state
    const inputResults = this.filterSearchResult()
    const filteredAppItems = this.filterTablistApps(inputResults)
    ...
    return (
      <div className="app-bg-container">
        <div className="topContainer">
          ...
          <ul className="Category-headers">
            {tabsList.map(eachTab => (
              <TabItem
                tabDetail={eachTab}
                key={eachTab.tabId}
                onChangeTabItems={this.onChangeTabItems}
                isActive={tabItemId === eachTab.tabId}
              />
            ))}
          </ul>
        </div>
        <ul className="appList-container">
          {filteredAppItems.map(eachApp => (
